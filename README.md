...
```
src/main/java/com/esyion/wechat/
├── Main.java                    # 示例入口
├── WechatChannelProvider.java    # 顶层API入口
├── WechatChannel.java           # Channel工厂
├── config/
│   └── Config.java              # 配置类
├── error/
...
└── channel/
    ├── ChannelTypes.java        # ChannelMsg/Reply类型
    ├── Inbound.java             # 接收消息处理
...
<configuration>
    ...
    <!-- SDK日志级别 -->
    <logger name="com.esyion.wechat" level="DEBUG"/>
</configuration>
```
...
```java
import com.esyion.wechat.*;
import com.esyion.wechat.wechat.*;
